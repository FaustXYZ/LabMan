...
<script language="JavaScript">
  ...
  // }
	function Inputcheck(checkapply){
		...
		// }
    // var std = getdate();
		if(checkapply.begind.value==""){
      // document.write(std);
			alert("Please Choose the start date！");
			return false;

		}
		if(checkapply.endd.value<=0) {
			alert("Please Choose the end date！");
			return false;
		}
    // var sub = new Date("Report.ddl.value").val().replace(/-/g,"/"));
	   if(checkapply.begind.value >checkapply.endd.value){
	   	alert("The end date must not be earlier than start date!");
			return false;
	   }
...
<title>TEST check execute page</title>	
<style type="text/css">
...
<form name="execute" form method="post" action="execute.php" enctype="multipart/form-data" onsubmit="return Inputcheck(this)">  
<div id="prosettitle" style="position:absolute;top:140px;left:420px">
      <font face="Times New Roman"><font size=6><font color=#00629b>Check execute</font></font></font></br>
</div>
...
<li><font size=4><font color=#00629b><font face="Times New Roman">Apply check:</font></font></font></br></li> </br>

<!-- <font color=#00629b><font size=2><font face="Times New Roman">ID:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspReID:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspName:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspQty:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspSpec:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspUnit price:&nbsp&nbsp&nbspDiscount:&nbsp&nbsp&nbspSalesman:&nbsp&nbspCell:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspCompany:&nbsp&nbspApplytime:</font></font></font></br> -->
    <?php
    $begind=$begind.' 00:00:00';
    // echo $begind;
    $endd=$endd.' 23:59:59';
    include("connectDB.php");
    // $result2=mysql_query("select *  from think_apply where Applytime >'$begind' and Applytime<'$endd' ");
    $result3=mysql_query("select * from think_apply where OrderID not in(select OrderID FROM think_checkapp where CheckID='$uid')");
    include("disconnectDB");
while($rs=mysql_fetch_array($result3)){  
  // $checkstatus = mysql_query("select * from think_checkapp where ID='$rs['ID']' and Subtime='rs$['Applytime']'");
  // $cr = mysql_fetch_array($checkstatus);
    ?>  
    <input type="text" name= "ID[]" style="" value="<?php echo 'ID: ',$rs['ID'];?>" readonly="readonly">
    <input type="text" name= "reID[]" style="" value="<?php echo 'ReID: ',$rs['ReID'];?>" readonly="readonly">
     <input type="text" name= "OrderID[]" style="" value="<?php echo $rs['OrderID'];?>" readonly="readonly">
    <input type="text" name= "name[]" style="" value="<?php echo 'Name: ',$rs['Agentsname'];?>" readonly="readonly">
    <input type="text" name= "num[]" style="" value="<?php echo 'Qty: ',$rs['AgentNum'];?>" readonly="readonly">
   <input type="text" name= "size[]" style="" value="<?php echo 'Spec: ',$rs['Agentsize'];?>" readonly="readonly">
   <input type="text" name= "price[]" style="" value="<?php echo 'UnitPrice: ',$rs['SinglePrize'];?>" readonly="readonly">
    <input type="text" name= "dis[]" style="" value="<?php echo 'Discount: ',$rs['Discount'];?>" readonly="readonly">
    <input type="text" name= "salesman[]" style="" value="<?php echo 'Sales: ',$rs['Saler'];?>" readonly="readonly">
    <input type="text" name= "salescon[]" style="" value="<?php echo 'Cell: ',$rs['Connect'];?>" readonly="readonly">
     <input type="text" name= "company[]" style="" value="<?php echo 'Company: ',$rs['Company'];?>" readonly="readonly">
     <input type="text" name= "subtime[]" style="" value="<?php echo 'Subtime: ',$rs['Applytime'];?>" readonly="readonly"></br>
     <input type="text" name= "remarks[]" style="width:120px;height: 40px" value="<?php echo'Remark: ',$rs['Remarks'];?>" readonly="readonly"></br>
    
     
     <font color=#00629b><font size=2><font face="Times New Roman">Action:</font></font></font>
     <select name="check[]"" style="width:90px">
       <!-- <option value=""></option> -->
    <option value="yes">Approve</option>
    <option value="no">Reject</option>
    <option value="wait" selected="selected">Not sure</option>
     </select>
      <input type="text" name= "comments[]" style="width:120px;height:40px" placeholder="comments"  ></br>
   </br></br>
    <!--  <input type="checkbox" name= "check[]" style="width:50px" value="<?php echo $rs['stud'];?>" ></br> -->
    <?php  
}  
?> 
</br>
<font size=2><font face="Times New Roman">Select <font color=red>'Approve'</font> for yes, <font color=red>'Reject'</font> for no and <font color=red>'Not sure'</font> for latter decision(as default);</br>The ones have been <u>Approved</u> or <u>Rejected</u>  will not be shown.</font></font></br></br>
